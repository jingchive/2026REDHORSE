import svgPaths from "../imports/svg-on6a90b19c";
import { CountdownTimer } from './CountdownTimer';

interface HorseCreate1Props {
  onHorseSelect: (horseId: number) => void;
  onNext: () => void;
...
export function HorseCreate1({ onHorseSelect, onNext, selectedHorse }: HorseCreate1Props) {
  return (
    <div className="bg-[#faf7f5] flex flex-col h-full w-full overflow-hidden" data-name="말 만들기 1">
      {/* Header */}
      <Nav />
      ...
      <div className="flex-1 flex flex-col items-center justify-start px-4 py-4 overflow-auto min-h-0">
        ...
        <div className="relative bg-white w-[230px] max-w-[90vw] shrink-0" style={{ aspectRatio: '230/577' }}>
          ...
          {/* Horse 1 - 중앙 */}
          <div 
            className={`absolute left-[50%] translate-x-[-50%] top-[36%] w-[61.7%] cursor-pointer transition-all`}
            onClick={() => onHorseSelect(1)}
            data-name="image 4 (Traced)"
            style={{ aspectRatio: '729.381/408.975' }}
          >
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 142 80">
              <g id="image 4 (Traced)">
                ...
                <path clipRule="evenodd" d={svgPaths.p2e1812f0} fill={selectedHorse === 1 ? "#A04842" : "none"} fillRule="evenodd" mask="url(#path-1-inside-1_3_831)" stroke="var(--stroke-0, #A04842)" strokeWidth="2" />
              </g>
            </svg>
          </div>
          ...
          {/* Horse 2 - 중하단 */}
          <div 
            className={`absolute left-[50%] translate-x-[-50%] top-[54%] w-[60%] cursor-pointer transition-all`}
            onClick={() => onHorseSelect(2)}
            data-name="running horse2"
            style={{ aspectRatio: '1031.7/595.261' }}
          >
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 138 80">
              <g id="running horse2">
                ...
                <path d={svgPaths.p288b6300} fill={selectedHorse === 2 ? "#A04842" : "none"} mask="url(#path-1-inside-1_3_829)" stroke="var(--stroke-0, #A04842)" strokeWidth="2" />
              </g>
            </svg>
          </div>
          ...
          {/* Horse 3 - 하단 */}
          <div 
            className={`absolute left-[50%] translate-x-[-50%] top-[73%] w-[57.4%] cursor-pointer transition-all`}
            onClick={() => onHorseSelect(3)}
            data-name="running horse3"
            style={{ aspectRatio: '1011.42/613.673' }}
          >
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 132 80">
              <g id="running horse3">
                ...
                <path d={svgPaths.p357e2d80} fill={selectedHorse === 3 ? "#A04842" : "none"} mask="url(#path-1-inside-1_3_833)" stroke="var(--stroke-0, #A04842)" strokeWidth="2" />
              </g>
            </svg>
          </div>
        </div>
